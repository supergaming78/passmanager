...
// (voir App.css).
//
// L'extension n'a PAS l'équivalent de la règle CSS `.theme-custom` (voir App.css côté desktop) :
// le seul `bg-white` en dur de la popup (UpdateBanner.tsx, un bouton SUR un bandeau `bg-indigo-
// 600`, pas un fond de page/carte) doit justement rester du vrai blanc, comme `text-white` sur les
// boutons de l'app desktop — rien à corriger ici. La classe `theme-custom` posée ci-dessous reste
// malgré tout pour cohérence/simplicité de duplication avec le fichier desktop.
//
...
//
// CORRECTIF (retour utilisateur : "ça reste très noir même les cases des mots de passe") — voir
// frontend(app)/public/theme-init.js pour le raisonnement détaillé (identique ici) : ce fichier
// était resté sur l'ANCIEN schéma "backgroundStyle" (3 valeurs discrètes) alors que
// lib/customTheme.ts est passé depuis à une saturation CONTINUE (0-100) par couleur, fond ET
// accents compris — jamais mis à jour ici lors de ce changement, donc systématiquement retombé sur
// un gris pur (chroma nulle) pour le fond, et 100% de saturation forcée pour les accents (jamais
// lue du tout). Cause probable du signalement : le popup recharge ce script à CHAQUE ouverture
// (contrairement à l'app desktop, dont le process persiste et masque le souci une fois corrigé par
// React) — ce mauvais rendu initial était donc bien plus visible/fréquent ici.
(function () {
  try {
