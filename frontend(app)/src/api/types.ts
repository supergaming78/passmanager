...
export interface MeResponse {
  email: string;
  /** Droits de modérateur (panneau Administration, gestion des comptes non-modérateur...). */
...
/** Payload admin pour PUT /admin/users/{email}/email — PAS de master_password_hash (l'admin ne
 * connaît pas, et ne peut pas connaître, le mot de passe maître de la cible). Ne touche jamais au
 * mot de passe maître ni à la clé du coffre, seulement l'identifiant email. */
export interface AdminUpdateEmailPayload {
  new_email: string;
}
...
/** Jamais `sealed_vault_key` ici (voir backend\src\models.rs::SharedVaultMemberView) : la clé
 * scellée d'un membre n'est déchiffrable que par lui. */
