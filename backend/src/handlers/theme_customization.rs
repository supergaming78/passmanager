...
pub async fn list_theme_profiles(State(state): State<Arc<AppState>>, user: AuthUser) -> Result<impl IntoResponse, AppError> {
    let profiles = ThemeProfileRepository::list(&state.db, user.user_id).await?;
    Ok(Json(profiles))
}
...
pub async fn create_theme_profile(
    ...
    let profile = ThemeProfileRepository::create(&state.db, user.user_id, &payload, is_admin).await?;
...
pub async fn update_theme_profile(
    ...
    payload.validate()?;
    let updated = ThemeProfileRepository::update(&state.db, user.user_id, &id, &payload).await?;
    if !updated {
...
pub async fn delete_theme_profile(State(state): State<Arc<AppState>>, user: AuthUser, Path(id): Path<String>) -> Result<impl IntoResponse, AppError> {
    let deleted = ThemeProfileRepository::delete(&state.db, user.user_id, &id).await?;
    if !deleted {
        return Err(AppError::NotFound);
    }
    Ok(StatusCode::NO_CONTENT)
...
pub async fn activate_theme_profile(State(state): State<Arc<AppState>>, user: AuthUser, Path(id): Path<String>) -> Result<impl IntoResponse, AppError> {
    let activated = ThemeProfileRepository::activate(&state.db, user.user_id, &id).await?;
    if !activated {
        return Err(AppError::NotFound);
    }
    Ok(StatusCode::NO_CONTENT)
...
pub async fn share_theme_profile(
    ...
    let shared_id = ThemeShareRepository::share(&state.db, user.user_id, &id, &shared_with_email).await?;
...
pub async fn list_shared_theme_profiles(State(state): State<Arc<AppState>>, user: AuthUser) -> Result<impl IntoResponse, AppError> {
    let shares = ThemeShareRepository::list_received(&state.db, user.user_id).await?;
    Ok(Json(shares))
}
...
pub async fn accept_shared_theme_profile(State(state): State<Arc<AppState>>, user: AuthUser, Path(id): Path<String>) -> Result<impl IntoResponse, AppError> {
    let is_admin = user.is_admin(&state);
    let created = ThemeShareRepository::accept(&state.db, &id, user.user_id, is_admin).await?;
...
pub async fn decline_shared_theme_profile(State(state): State<Arc<AppState>>, user: AuthUser, Path(id): Path<String>) -> Result<impl IntoResponse, AppError> {
    let declined = ThemeShareRepository::decline(&state.db, &id, user.user_id).await?;
    if !declined {
        return Err(AppError::NotFound);
    }
    Ok(StatusCode::NO_CONTENT)
...
pub async fn update_preferred_theme(
    ...
    sqlx::query("UPDATE users SET preferred_theme = ? WHERE id = ?")
        .bind(&payload.theme)
        .bind(user.user_id)
        .execute(&state.db)
        .await?;
...
mod tests {
    ...
    /// -1 si l'email n'a jamais été enregistré (voir test_list_is_empty_when_never_configured et
    /// les tests de validation, qui échouent avant tout accès BDD) : une requête WHERE user_id = -1
    /// ne matche simplement aucune ligne, exactement le résultat "vide" attendu dans ces cas.
    ...
    async fn test_list_is_empty_when_never_configured() {
        ...
        let response = list_theme_profiles(State(state.clone()), auth(&state, "jamais-configure@example.com").await).await.unwrap();
    ...
    async fn test_create_then_list_round_trips() {
        ...
        create_theme_profile(State(state.clone()), auth(&state, email).await, Json(sample_payload("Mon profil"))).await.unwrap();

        let response = list_theme_profiles(State(state.clone()), auth(&state, email).await).await.unwrap();
        let body = read_json_body(response.into_response()).await;
    ...
    async fn test_non_admin_capped_at_three_profiles() {
        ...
        for i in 0..3 {
            create_theme_profile(State(state.clone()), auth(&state, email).await, Json(sample_payload(&format!("Profil {i}")))).await.unwrap();
        }
        let result = create_theme_profile(State(state.clone()), auth(&state, email).await, Json(sample_payload("Profil 4"))).await;
    ...
    async fn test_admin_has_no_profile_limit() {
        ...
        for i in 0..5 {
            create_theme_profile(State(state.clone()), auth(&state, admin_email).await, Json(sample_payload(&format!("Profil {i}")))).await.unwrap();
        }

        let response = list_theme_profiles(State(state.clone()), auth(&state, admin_email).await).await.unwrap();
        let body = read_json_body(response.into_response()).await;
    ...
    async fn test_profiles_are_isolated_between_accounts() {
        ...
        create_theme_profile(State(state.clone()), auth(&state, "a@example.com").await, Json(sample_payload("Profil A"))).await.unwrap();

        let response = list_theme_profiles(State(state.clone()), auth(&state, "b@example.com").await).await.unwrap();
        let body = read_json_body(response.into_response()).await;
    ...
    async fn test_create_rejects_out_of_range_hue() {
        ...
        let result = create_theme_profile(State(state.clone()), auth(&state, "user3@example.com").await, Json(payload)).await;
        assert!(matches!(result, Err(AppError::ValidationError(_))));
    }
    ...
    async fn test_create_rejects_out_of_range_lightness() {
        ...
        let result = create_theme_profile(State(state.clone()), auth(&state, "user4@example.com").await, Json(payload)).await;
        assert!(matches!(result, Err(AppError::ValidationError(_))));
    }
    ...
    async fn test_create_rejects_out_of_range_saturation() {
        ...
        let result = create_theme_profile(State(state.clone()), auth(&state, "user7@example.com").await, Json(payload)).await;
        assert!(matches!(result, Err(AppError::ValidationError(_))));
    }
    ...
    async fn test_activate_deactivates_other_profiles() {
        ...
        let r1 = create_theme_profile(State(state.clone()), auth(&state, email).await, Json(sample_payload("Profil 1"))).await.unwrap();
        let body1 = read_json_body(r1.into_response()).await;
        let id1 = body1["id"].as_str().unwrap().to_string();

        let r2 = create_theme_profile(State(state.clone()), auth(&state, email).await, Json(sample_payload("Profil 2"))).await.unwrap();
        let body2 = read_json_body(r2.into_response()).await;
        let id2 = body2["id"].as_str().unwrap().to_string();

        activate_theme_profile(State(state.clone()), auth(&state, email).await, Path(id1.clone())).await.unwrap();
        activate_theme_profile(State(state.clone()), auth(&state, email).await, Path(id2.clone())).await.unwrap();
    ...
    async fn test_cannot_activate_update_or_delete_another_accounts_profile() {
        ...
        let created = create_theme_profile(State(state.clone()), auth(&state, "a2@example.com").await, Json(sample_payload("Profil A"))).await.unwrap();
        let body = read_json_body(created.into_response()).await;
        let id = body["id"].as_str().unwrap().to_string();

        let activate_result = activate_theme_profile(State(state.clone()), auth(&state, "b2@example.com").await, Path(id.clone())).await;
        assert!(matches!(activate_result, Err(AppError::NotFound)));

        let update_result = update_theme_profile(State(state.clone()), auth(&state, "b2@example.com").await, Path(id.clone()), Json(sample_payload("Piraté"))).await;
        assert!(matches!(update_result, Err(AppError::NotFound)));

        let delete_result = delete_theme_profile(State(state.clone()), auth(&state, "b2@example.com").await, Path(id.clone())).await;
    ...
    async fn test_delete_nonexistent_profile_returns_not_found() {
        let state = build_test_state(None).await;
        let result = delete_theme_profile(State(state.clone()), auth(&state, "user6@example.com").await, Path("id-inexistant".to_string())).await;
    ...
    async fn test_share_accept_lifecycle() {
        ...
        let created = create_theme_profile(State(state.clone()), auth(&state, "sender@example.com").await, Json(sample_payload("Mon thème"))).await.unwrap();
        let profile_id = read_json_body(created.into_response()).await["id"].as_str().unwrap().to_string();

        let share_result = share_theme_profile(
            State(state.clone()),
            auth(&state, "sender@example.com").await,
            Path(profile_id),
        ...
        let received = list_shared_theme_profiles(State(state.clone()), auth(&state, "recipient@example.com").await).await.unwrap();
        ...
        let accepted = accept_shared_theme_profile(State(state.clone()), auth(&state, "recipient@example.com").await, Path(share_id)).await.unwrap();
        ...
        let recipient_profiles = list_theme_profiles(State(state.clone()), auth(&state, "recipient@example.com").await).await.unwrap();
        ...
        let received_after = list_shared_theme_profiles(State(state.clone()), auth(&state, "recipient@example.com").await).await.unwrap();
    ...
    async fn test_cannot_share_with_self() {
        ...
        let created = create_theme_profile(State(state.clone()), auth(&state, "solo@example.com").await, Json(sample_payload("Profil"))).await.unwrap();
        let profile_id = read_json_body(created.into_response()).await["id"].as_str().unwrap().to_string();

        let result = share_theme_profile(
            State(state.clone()),
            auth(&state, "solo@example.com").await,
            Path(profile_id),
    ...
    async fn test_cannot_share_another_users_profile() {
        ...
        let created = create_theme_profile(State(state.clone()), auth(&state, "realowner3@example.com").await, Json(sample_payload("Profil"))).await.unwrap();
        let profile_id = read_json_body(created.into_response()).await["id"].as_str().unwrap().to_string();

        let result = share_theme_profile(
            State(state.clone()),
            auth(&state, "attacker3@example.com").await,
            Path(profile_id),
    ...
    async fn test_share_with_nonexistent_recipient_fails() {
        ...
        let created = create_theme_profile(State(state.clone()), auth(&state, "sender2@example.com").await, Json(sample_payload("Profil"))).await.unwrap();
        let profile_id = read_json_body(created.into_response()).await["id"].as_str().unwrap().to_string();

        let result = share_theme_profile(
            State(state.clone()),
            auth(&state, "sender2@example.com").await,
            Path(profile_id),
    ...
    async fn test_only_recipient_can_accept_share() {
        ...
        let created = create_theme_profile(State(state.clone()), auth(&state, "sender3@example.com").await, Json(sample_payload("Profil"))).await.unwrap();
        let profile_id = read_json_body(created.into_response()).await["id"].as_str().unwrap().to_string();
        let share_result = share_theme_profile(
            State(state.clone()),
            auth(&state, "sender3@example.com").await,
            Path(profile_id),
        ...
        let stranger_attempt = accept_shared_theme_profile(State(state.clone()), auth(&state, "stranger3@example.com").await, Path(share_id.clone())).await;
        assert!(matches!(stranger_attempt, Err(AppError::NotFound)));

        let sender_attempt = accept_shared_theme_profile(State(state.clone()), auth(&state, "sender3@example.com").await, Path(share_id)).await;
    ...
    async fn test_decline_share_either_side() {
        ...
        let created = create_theme_profile(State(state.clone()), auth(&state, "sender4@example.com").await, Json(sample_payload("Profil"))).await.unwrap();
        let profile_id = read_json_body(created.into_response()).await["id"].as_str().unwrap().to_string();

        async fn make_share(state: &Arc<AppState>, profile_id: &str) -> String {
            let result = share_theme_profile(
                State(state.clone()),
                auth(&state, "sender4@example.com").await,
        ...
        let stranger_attempt = decline_shared_theme_profile(State(state.clone()), auth(&state, "stranger4@example.com").await, Path(share_id_1.clone())).await;
        assert!(matches!(stranger_attempt, Err(AppError::NotFound)));

        // Le destinataire peut décliner.
        decline_shared_theme_profile(State(state.clone()), auth(&state, "recipient4@example.com").await, Path(share_id_1)).await.unwrap();

        // L'expéditeur peut annuler.
        let share_id_2 = make_share(&state, &profile_id).await;
        decline_shared_theme_profile(State(state.clone()), auth(&state, "sender4@example.com").await, Path(share_id_2)).await.unwrap();

        let received = list_shared_theme_profiles(State(state.clone()), auth(&state, "recipient4@example.com").await).await.unwrap();
        let received_body = read_json_body(received.into_response()).await;
    ...
    async fn test_accept_share_respects_profile_limit() {
        ...
        for i in 0..3 {
            create_theme_profile(State(state.clone()), auth(&state, "recipient5@example.com").await, Json(sample_payload(&format!("Profil {i}")))).await.unwrap();
        }
        let created = create_theme_profile(State(state.clone()), auth(&state, "sender5@example.com").await, Json(sample_payload("Cadeau"))).await.unwrap();
        let profile_id = read_json_body(created.into_response()).await["id"].as_str().unwrap().to_string();
        let share_result = share_theme_profile(
            State(state.clone()),
            auth(&state, "sender5@example.com").await,
            Path(profile_id),
        ...
        let accept_result = accept_shared_theme_profile(State(state.clone()), auth(&state, "recipient5@example.com").await, Path(share_id.clone())).await;
        assert!(matches!(accept_result, Err(AppError::ValidationError(_))));

        // Toujours en attente, pas perdu.
        let received = list_shared_theme_profiles(State(state.clone()), auth(&state, "recipient5@example.com").await).await.unwrap();
        let received_body = read_json_body(received.into_response()).await;
    ...
    async fn test_update_preferred_theme_persists_and_is_readable_via_get_me() {
        ...
        update_preferred_theme(State(state.clone()), auth(&state, email).await, Json(UpdatePreferredThemePayload { theme: "midnight".to_string() }))
            .await
    ...
    async fn test_update_preferred_theme_rejects_unknown_value() {
        ...
        let result = update_preferred_theme(State(state.clone()), auth(&state, email).await, Json(UpdatePreferredThemePayload { theme: "not-a-real-theme".to_string() })).await;
        assert!(matches!(result, Err(AppError::ValidationError(_))));
